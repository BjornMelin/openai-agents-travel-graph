...
async def create_test_data(client: SupabaseClient) -> None:
    ...
    # Create a test user
    user_result = await client.client.table("users").insert({
    ...
    query_result = await client.client.table("travel_queries").insert({
        "user_id": user_id,
    ...
    await client.client.table("user_preferences").insert({
        "user_id": user_id,
...
async def main(args: argparse.Namespace) -> int:
    ...
    if not config.api.validate():
        ...
        logger.error("Please ensure SUPABASE_URL and SUPABASE_KEY are set in your environment")
        return 1
...
def parse_args() -> argparse.Namespace:
    ...
    Returns:
        Parsed arguments
    """
    parser = argparse.ArgumentParser(description="Set up the Supabase database for the travel planner")
