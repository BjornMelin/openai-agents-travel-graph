...
class SupabaseClient:
    ...
    async def save_user_preferences(
        ...
        # Save to database
        result = await self.client.table("user_preferences").insert(preferences_dict).execute()
    ...
    async def get_travel_plan(self, plan_id: str) -> TravelPlan | None:
        ...
        """
        result = await self.client.table("travel_plans").select("*").eq("id", plan_id).execute()
    ...
    async def get_user_travel_plans(self, user_id: str) -> list[dict[str, Any]]:
        ...
        """
        result = await self.client.table("travel_plans").select("*").eq("user_id", user_id).execute()
    ...
    async def get_travel_query(self, query_id: str) -> TravelQuery | None:
        ...
        """
        result = await self.client.table("travel_queries").select("*").eq("id", query_id).execute()
        ...
        if query_data.get("departure_date"):
            query_data["departure_date"] = date.fromisoformat(query_data["departure_date"])
    ...
    async def delete_travel_plan(self, plan_id: str) -> bool:
        ...
        """
        result = await self.client.table("travel_plans").delete().eq("id", plan_id).execute()
        
        return len(result.data) > 0
        
    async def update_travel_plan(self, plan_id: str, plan: TravelPlan) -> dict[str, Any]:
        """
        ...
        # Update in database
        result = await self.client.table("travel_plans").update(plan_dict).eq("id", plan_id).execute()
...
class SupabaseRepository(Generic[T]):
    ...
    def __init__(self, supabase_client: SupabaseClient, table_name: str, model_class: type[T]):
        """
    ...
    async def create(self, item: T, **extra_fields) -> T:
        ...
        # Save to database
        result = await self.client.client.table(self.table_name).insert(item_dict).execute()
    ...
    async def get_by_id(self, item_id: str) -> T | None:
        ...
        """
        result = await self.client.client.table(self.table_name).select("*").eq("id", item_id).execute()
    ...
    async def update(self, item_id: str, item: T) -> T:
        ...
        # Update in database
        result = await self.client.client.table(self.table_name).update(item_dict).eq("id", item_id).execute()
    ...
    async def delete(self, item_id: str) -> bool:
        ...
        Returns:
            True if deletion was successful
        """
        result = await self.client.client.table(self.table_name).delete().eq("id", item_id).execute()
