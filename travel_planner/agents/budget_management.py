...
class BudgetManagementAgent(BaseAgent[BudgetContext]):
        ...
        if context.category_preferences:
            ...
            if total_points == 0:
                ...
                for category in ExpenseCategory:
                    ...
                    adjusted_percentages[category] = max(1, base_percentage * adjustment_factor)
                
                # Normalize percentages to sum to 100
                total_percentage = sum(adjusted_percentages.values())
                for category in adjusted_percentages:
                    adjusted_percentages[category] = (adjusted_percentages[category] / total_percentage) * 100
        else:
            adjusted_percentages = default_percentages
    ...
    async def _extract_expenses(self, input_data: str | list[dict[str, Any]], context: BudgetContext) -> list[BudgetItem]:
        ...
        if "hotel" in user_input.lower() or "stay" in user_input.lower() or "accommodation" in user_input.lower():
            new_expenses.append(BudgetItem(
        ...
        if "flight" in user_input.lower() or "airline" in user_input.lower() or "plane" in user_input.lower():
            new_expenses.append(BudgetItem(
                ...
                is_required=True,
                alternatives=[
                    {"name": "Budget airline", "amount": 700.0, "currency": "USD", "note": "Less convenient times, 1 stop"}
                ]
            ))
        
        if "tour" in user_input.lower() or "activity" in user_input.lower() or "visit" in user_input.lower():
            new_expenses.append(BudgetItem(
                ...
                is_required=False,
                alternatives=[
                    {"name": "Standard Seine Cruise", "amount": 25.0, "currency": "USD", "note": "Without dinner, 1-hour tour"}
                ]
            ))
    ...
    async def _check_budget_alerts(self, context: BudgetContext) -> None:
        ...
        for category, allocation in context.allocations.items():
            ...
            if allocation.total_spent > (allocation.amount * 0.8) and allocation.total_spent <= allocation.amount:
                context.alerts.append(
    ...
    async def _generate_budget_report(self, context: BudgetContext) -> str:
        ...
        alerts_details = "\n".join(context.alerts) if context.alerts else "No budget alerts at this time."
