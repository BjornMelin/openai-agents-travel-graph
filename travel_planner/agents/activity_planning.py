...
class ActivityPlanningAgent(BaseAgent[ActivityPlanningContext]):
    ...
    def __init__(self, config: Optional[AgentConfig] = None):
        ...
        default_config = AgentConfig(
            ...
            itineraries that account for location, opening hours, travel time, and budget.
            """,
            model="gpt-4o",
    ...
    async def _research_activities(self, context: ActivityPlanningContext) -> List[Activity]:
        ...
        mock_activities = [
            ...
            Activity(
                ...
                description="Sightseeing cruise along the Seine River with views of Paris landmarks.",
                price=15.0,
                currency="EUR",
                duration_minutes=60,
                opening_hours={
        ...
        for activity in mock_activities:
            if activity.type not in context.excluded_activity_types:
                if not context.interests or any(tag in context.interests for tag in activity.tags):
                    filtered_activities.append(activity)
        
        # If no activities match the interests, return all non-excluded activities
        if not filtered_activities:
            filtered_activities = [act for act in mock_activities if act.type not in context.excluded_activity_types]
    ...
    async def _get_weather_forecasts(self, context: ActivityPlanningContext) -> Dict[str, Dict[str, Any]]:
        ...
        while current_date <= end:
            ...
            forecasts[date_str] = {
                "condition": WeatherCondition.SUNNY if is_sunny else WeatherCondition.CLOUDY,
    ...
    async def _create_daily_itineraries(self, context: ActivityPlanningContext) -> Dict[str, DailyItinerary]:
        ...
        """
        if not context.start_date or not context.end_date or not context.available_activities:
            return {}
        ...
        while current_date <= end:
            ...
            for activity in context.available_activities:
                ...
                if opening_hours and (opening_hours.get("open") != time(0, 0) or opening_hours.get("close") != time(0, 0)):
                    ...
                    elif weather and weather.get("condition") in activity.suitable_weather:
                        suitable_activities.append(activity)
            ...
            morning_activities = [a for a in suitable_activities if a.type in [ActivityType.MUSEUM, ActivityType.ATTRACTION, ActivityType.TOUR]]
            if morning_activities and len(itinerary.activities) < 3:
                selected = morning_activities[0]  # In a real implementation, we would make a smarter selection
                suitable_activities.remove(selected)
                
                itinerary.activities.append(ScheduledActivity(
                    activity=selected,
                    date=date_str,
                    start_time="09:00",
                    end_time="12:00",
            ...
            afternoon_activities = [a for a in suitable_activities if a.type in [ActivityType.OUTDOOR, ActivityType.CULTURAL, ActivityType.SHOPPING]]
            if afternoon_activities and len(itinerary.activities) < 3:
                selected = afternoon_activities[0]  # In a real implementation, we would make a smarter selection
                suitable_activities.remove(selected)
            ...
            evening_activities = [a for a in suitable_activities if a.type in [ActivityType.FOOD, ActivityType.ENTERTAINMENT]]
            if evening_activities and len(itinerary.activities) < 3:
                selected = evening_activities[0]  # In a real implementation, we would make a smarter selection
            ...
            if context.budget_per_day and itinerary.total_cost > context.budget_per_day:
                ...
                itinerary.notes += f"\nNote: This day's activities exceed your daily budget of {context.budget_per_day} EUR."
    ...
    async def _generate_itinerary_summary(self, context: ActivityPlanningContext) -> str:
        """
        ...
        for date_str, itinerary in context.daily_itineraries.items():
            ...
            for i, scheduled in enumerate(itinerary.activities, 1):
                ...
                day_details.append(
                    f"{i}. {activity.name} ({scheduled.start_time} - {scheduled.end_time})\n"
                    f"   Type: {activity.type.value}, Price: {activity.formatted_price}\n"
