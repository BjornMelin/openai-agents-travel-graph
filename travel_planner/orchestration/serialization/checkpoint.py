...
class CheckpointManager:
            ...
            try:
                with open(checkpoint_path) as f:
                    ...
                    checkpoint_info = {
                        ...
                        "plan_id": metadata.get("plan", {}).get("metadata", {}).get("id"),
                        "destination": metadata.get("plan", {}).get("destination", {}).get("name"),
    ...
    def _reconstruct_state_from_checkpoint(self, 
        ...
        preferences = UserPreferences.model_validate(preferences_data) if preferences_data else None
        
        # Convert string stage to enum
        current_stage_str = checkpoint_data.get("current_stage", "start")
        current_stage = WorkflowStage(current_stage_str) if current_stage_str else WorkflowStage.START
        
        # Extract previous stage if available
        previous_stage_str = checkpoint_data.get("previous_stage")
        previous_stage = WorkflowStage(previous_stage_str) if previous_stage_str else None
        ...
        state = TravelPlanningState(
            ...
            start_time=datetime.fromisoformat(checkpoint_data.get("start_time")) if checkpoint_data.get("start_time") else None,
            last_update_time=datetime.fromisoformat(checkpoint_data.get("last_update_time")) if checkpoint_data.get("last_update_time") else None,
        )
