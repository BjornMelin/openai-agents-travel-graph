...
class IncrementalCheckpointManager:
    ...
    def __init__(self, base_manager: CheckpointManager | None = None):
        ...
        self.chain_length = 0
        self.max_chain_length = 5  # Maximum number of incremental checkpoints before a full one
    ...
    def save_checkpoint(self, state: TravelPlanningState) -> str:
        ...
        checkpoint_id = f"incr_{state.current_stage.value}_{datetime.now().strftime('%Y%m%d%H%M%S')}_{state.plan.id if state.plan else 'noplan'}"
        
        # Check if we need a full checkpoint
        if not self.last_full_checkpoint_id or self.chain_length >= self.max_chain_length:
        ...
        if self.last_full_checkpoint_id:
            ...
            previous_state = self.base_manager.load_checkpoint(self.last_full_checkpoint_id)
            ...
            self.chain_length += 1
            logger.info(f"Created incremental checkpoint: {checkpoint_id} (based on {self.last_full_checkpoint_id})")
            return checkpoint_id
        else:
    ...
    def _calculate_state_diff(self, base_state: TravelPlanningState, 
                           current_state: TravelPlanningState) -> dict[str, Any]:
        """
    ...
    def _apply_state_diff(self, base_state: TravelPlanningState, 
                        diff: dict[str, Any]) -> TravelPlanningState:
        """
