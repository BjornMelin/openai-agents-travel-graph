...
class FlightSearchAutomation(BrowserAutomation):
    ...
    def __init__(
        ...
        """
        super().__init__(headless=headless, slow_mo=slow_mo, timeout=timeout, use_cache=use_cache)
        
        if provider not in self.SEARCH_SITES:
            raise ValueError(f"Unsupported provider: {provider}. Available providers: {list(self.SEARCH_SITES.keys())}")
    ...
    async def search_flights(
        ...
        try:
            ...
            error_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "error_screenshots")
            os.makedirs(error_dir, exist_ok=True)
            screenshot_path = os.path.join(error_dir, f"flight_search_error_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png")
    ...
    async def _handle_google_flights_specifics(self, departure_date: date, return_date: date | None = None):
        """
    ...
    async def _handle_skyscanner_specifics(self):
        ...
        try:
            ...
            consent_button = await self.page.query_selector("button[id*='cookie-consent'], button[id*='accept-cookies']")
            if consent_button:
    ...
    async def _extract_flight_results(
        ...
        """
        logger.info(f"Extracting up to {max_results} flight results sorted by {sort_by}")
        ...
        extraction_result = await self.extract_data(
            extraction_prompt=extract_prompt.format(max_results=max_results),
            context={}
        )
    ...
    def _parse_extraction_to_flights(self, extraction_result: dict[str, Any]) -> list[Flight]:
        """
