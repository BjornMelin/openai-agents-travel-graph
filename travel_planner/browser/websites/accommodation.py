...
class AccommodationSearchAutomation(BrowserAutomation):
    ...
    async def search_accommodations(
        ...
        try:
            ...
            screenshot_path = os.path.join(error_dir, f"accommodation_search_error_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png")
    ...
    async def _apply_filters(
        ...
        if accommodation_type:
            filter_instructions.append(f"- Accommodation Type: {accommodation_type.value}")
    ...
    async def _extract_accommodation_results(self, max_results: int = 5) -> list[Accommodation]:
        """
    ...
    def _parse_extraction_to_accommodations(self, extraction_result: dict[str, Any]) -> list[Accommodation]:
        """
