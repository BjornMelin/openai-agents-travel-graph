...
class ElementNotFoundRecoveryStrategy(BrowserRecoveryStrategy):
    ...
    async def handle(self, error: Exception, page: Page | None = None) -> bool:
        ...
        try:
            ...
            frames = page.frames()
            if len(frames) > 1:
                logger.info(f"Page has {len(frames)} frames, checking for elements in frames")
        ...
        async def wrapper(*args, **kwargs):
            ...
            for attempt in range(max_attempts):
                try:
                    ...
                    last_error = e
                    logger.error(f"Error in {func.__name__}, attempt {attempt + 1}/{max_attempts}: {e!s}")
